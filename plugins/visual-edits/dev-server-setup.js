...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    // Do NOT add express.json() globally — it consumes POST bodies and breaks /api proxy.
      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(`git -c user.name="visual-edit" -c user.email="noreply@shandecors.store" add "${result.file}"`);
              execSync(`git -c user.name="visual-edit" -c user.email="noreply@shandecors.store" commit -m "visual_edit_variable_${timestamp}"`);
            } catch (gitError) {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="noreply@shandecors.store" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="noreply@shandecors.store" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
